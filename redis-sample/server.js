...
function cache(key, ttl, slowFn) {
    return async function cachedFn(...props) {
        ...
        if (cachedResponse) {
            console.log("It's cached already")
...
async function verySlowAndExpresivePostgreSQLQuery() {
    //Here you would do a big ugly query for postgresSQL

    console.log('This is a very expensive query');
...
const cachedFn = cache('Expensive_call', 10, verySlowAndExpresivePostgreSQLQuery);
